    ...
    LaunchedEffect(questionId) {
        db.collection("questions")
            ...
            .addOnSuccessListener { questionDocument ->
                correctAnswers =
                    questionDocument.get("correctAnswers") as? List<String> ?: emptyList()
                ...
                db.collection("responses")
                    ...
                    .addOnSuccessListener { documents ->
                        val responses =
                            documents.mapNotNull { it.data["selectedAnswer"] as? String }

                        val resultCounts =
                            questionOptions.associateWith { option -> responses.count { it == option } }
        ...
        db.collection("responses")
            ...
            .addOnSuccessListener { documents ->
                val responses = documents.flatMap {
                    ...
                    (it.data["selectedAnswers"] as? List<*>)?.filterIsInstance<String>()
                        ?: emptyList()
                }
                correctAnswers =
                    documents.mapNotNull { it.data["correctAnswers"] as? List<String> }.flatten()
        ...
        db.collection("responses")
            ...
            .addOnSuccessListener { documents ->
                ...
                selectedAnswers = documents.flatMap {
                    val selectedPairs =
                        it.data["selectedAnswers"] as? List<Map<String, String>> ?: emptyList()
                ...
                correctAnswers = documents.flatMap {
                    val correctPairs =
                        it.data["correctAnswers"] as? List<Map<String, String>> ?: emptyList()
    ...
    LaunchedEffect(questionId) {
        db.collection("responses")
            ...
            .addOnSuccessListener { documents ->
                ...
                correctAnswers =
                    documents.firstOrNull()?.get("correctAnswers") as? List<String> ?: emptyList()
                isLoading = false
            }
            .addOnFailureListener { e ->
                Log.e("Firebase", "Error fetching results", e)
                    ...
                    selectedAnswers.forEach { answers ->
                        ...
                        Row(
                            ...
                            modifier = Modifier
                                ...
                                .padding(vertical = 4.dp)
                                .background(
                                    if (isCorrectAnswer) Color(0xFFDFF0D8) else Color(
                                        0xFFF2DEDE
                                    )
                                )
                                .padding(8.dp)
...
fun PieChart(
    ...
    Canvas(modifier = modifier.size(size.dp)) {
        ...
        data.forEachIndexed { index, value ->
            ...
            val labelX =
                size / 2 + radius / 2 * cos(Math.toRadians(middleAngle.toDouble())).toFloat()
            val labelY =
                size / 2 + radius / 2 * sin(Math.toRadians(middleAngle.toDouble())).toFloat()
